...
alpha  :  Coefficient for Tikhonov Regularization. Alpha is the coefficient for second-order tikhonov regularization if both zeroth- and second-order regularization are applied
    Default is 1

beta  :  Coefficient for Zeroth-Order Tikhonov Regularization. Used if zeroth- and second-order Tikhonov regularization are applied
    Default is 0

tikhonov  :  Order of Tikhonov Regularization to Apply (Options: 'zeroth', 'second')
    Default is zeroth

reference_height  :  Reference Load Height used to Generate the Jacobian (units: m) [see run_jc.py]
    Default is 1

reference_density  :  Reference Load Density used to Generate the Jacobian (units: kg/m^3) [see run_jc.py]
    Default is 1000
...
apriori  : Dataset containing a priori hydrologic data [NOTE: It is assumed that a priori hydrologic data is aligned with load cells; being solved for]
    Default is False (assumed that a priori data is not provided)

weighted  : Dataset contains uncertainity estimates to be used in a weighted inversion
    Default is False (assumed that uncertainity estimates are not provided)
...
def main(dm_file,data_files,apriori_file,rank,procN,comm,reference_height=1.,reference_density=1000.,alpha=1.,beta=0.,tikhonov='zeroth',outfile=".txt",uonly=False,apriori=False,weighted=False):
    ...
    if (rank == 0):
        ...
        sigma_vector = np.empty((len(file_ids),len(load_cell_ids)))
    ...
    # Create a Data Type for the Sigma Vector
    sig_size = len(load_cell_ids)
    sigtype = MPI.DOUBLE.Create_contiguous(sig_size)
    sigtype.Commit()
    ...
    if comm.rank != 0:
        design_matrix = np.empty(shape, dtype=dtype)
    comm.Bcast(design_matrix, root=0)
    ...
    sig_sub = np.empty((len(d_sub),mv_size))
    for ii in range(0,len(d_sub)):
        ...
        mv_sub[ii,:],sig_sub[ii,:] = perform_inversion.main(mfile,file_id,apriori_file,design_matrix,sta_ids,sta_comp_ids,sta_comp_lat,sta_comp_lon,load_cell_ids,load_cell_lat,load_cell_lon,alpha,beta,tikhonov,uonly=uonly,apriori=apriori,weighted=weighted)

    # Gather Results
    comm.Gatherv(mv_sub, [model_vector, (sendcounts, None), mvtype], root=0)
    comm.Gatherv(sig_sub, [sigma_vector, (sendcounts, None), sigtype], root=0)
    ...
    if (rank == 0):
        ...
        sigma_vector = sigma_vector[nidx,:]
        #print(model_vector)
        ...
        for kk in range(0,len(data_files)):
            ...
            csig = sigma_vector[kk,:]

            # Compute load pressure in 1000*N/m^2 (kPa)
            # NOTE: Thinking about whether we should automatically compute pressure or not. Commented out for now...
            ...
            # convert alpha and beta to strings
            alpha = str(alpha)
            beta = str(beta) 
            #print("outfile",outfile)
            #print("cfileid before error",cfileid)
            outfile = "_TikReg-" +tikhonov+ "-" +beta+ "-" +alpha+ ".txt"
            #print("outfile",outfile)
            ...
            all_inv_data = np.array(list(zip(load_cell_lat,load_cell_lon,cmv_h,csig)), dtype=[('load_cell_lat',float),('load_cell_lon',float),('cmv_h',float),('csig',float)])
  
            # Write Header Info to File
            hf = open(inv_head,'w')
            #inv_str = 'LoadCellID  LoadCellLat(+N,deg)  LoadCellLon(+E,deg)  LoadCellHeight(m)  LoadCellPressure(kPa)  \n'
            inv_str = 'LoadCellLat(+N,deg)  LoadCellLon(+E,deg)  LoadCellHeight(m)  95%CI(m)  \n'
            hf.write(inv_str)
            hf.close()
